...
function draw_gover()
	...
	print("press ❎",46,90,5+sin(time())/2)
...
function hitmob(atkm,defm,rawdmg)
 ...
 defm.bless=0
	...
	local def=defm.defmin+flr(rnd(
		defm.defmax-defm.defmin+1))
...
function unfogtile(x,y)
	...
	if iswalkable(x,y,"sight") then
		for i=1,4 do
			...
			if inbounds(tx,ty)
			and not iswalkable(tx,ty,"sight") then
				fog[tx][ty]=0
			end
		end		
	end
...
function throw()
	local itm,
							tx,ty=
								...
								throwtile()
	sfx(52)							
--	debug[2]="slot:"..thrslt.." "..itm_type[itm]
--debug[1]="yes throw"
...
function showinv()
	...
	statwind=addwind(5,5,84,13,
		{txt..
		"atk:"..p_mob.atk.. 
		 " def:"..p_mob.defmin.."-"..p_mob.defmax}
	)
...
function showuse()
	local itm=invwind.cur<3 
		and eqp[invwind.cur]
		or inv[invwind.cur-3]
 if itm==nil then 
 	return 
 end
 local typ,txt=
...
function triguse()
	...
	_upd,inv[i-3],p_mob.mov,back,p_t=
		update_pturn,
		nil,
		nil,
		false,
...
function showhint()
	...
	if invwind.cur>3 then
		...
		if itm and itm_type[itm]=="fud" then
			local txt=itm_known[itm] 
			and itm_name[itm]..itm_desc[itm] 
			or "???"
...
function mov_walk(this)
	local tme=1-p_t
	this.ox=this.sox*tme
	this.oy=this.soy*tme
end
...
function ai_attac(m)
	if dist(m.x,m.y,p_mob.x,p_mob.y)
		==1 then
			--attack player
...
function cansee(m1,m2)
	return dist(m1.x,m1.y,m2.x,m2.y)<=m1.los 
...
--	debug[1]=shl(0b101,2)
--	debug[2]=0b10100
--	debug[3]=bor(0b11,0b11)
--	debug[4]=bxor(0b11,0b10)
--	debug[5]=band(0b0,0b0)
-- debug[2]=stat(0)
...
function mazeworm()
	repeat
		...
		for _x=0,15 do
			for _y=0,15 do
				if cancarve(_x,_y,false)
				and not nexttoroom(_x,_y)	then
					add(cand,{x=_x,y=_y})
--					mset(_x,_y,3)
--				else
--					mset(_x,_y,2)				 
...
function digworm(x,y)
	...
	repeat
		mset(x,y,1)
--		snapshot()
	 if not cancarve(x+dirx[dr],y+diry[dr],false) 
	 or (rnd()<0.5 and step>2) then
			step=0
			local cand={}
			for i=1,4 do
	 ...
	 filled=false
		for _x=0,15 do
			for _y=0,15 do
			 tle=mget(_x,_y)
			 --★
				if cancarve(_x,_y,true)
				and tle!=14 and tle!=15 then
					filled=true
					mset(_x,_y,2)
--					snapshot()
				end
...
function isdoor(x,y)
	...
	or bcomp(sig,0b00110000,0b00001111)
	then
...
function nexttoroom(x,y,dirs)
 local dirs=dirs or 4
	for i=1,dirs do
		if inbounds(x+dirx[i],y+diry[i])
		and roomap[x+dirx[i]][y+diry[i]]!=0 
		then
			return true
		end
	end
...
function next2tile(_x,_y,tle)
	for i=1,4 do
		if inbounds(_x+dirx[i],_y+diry[i])
		and mget(_x+dirx[i],_y+diry[i])==tle then
			return true			
		end
	end
	return false
...
function deco_torch(r,tx,ty,x,y)
	if rnd(3)>1 and y%2==1 
	and not next2tile(tx,ty,13) then
		if x==0 then
			mset(tx,ty,64)
		elseif x==r.w-1 then
			mset(tx,ty,66)
...
function spawnchests()
	local chestdice,rpot,rare,place=
								explodeval("0,1,1,1,2,3"),
								{},
								true
